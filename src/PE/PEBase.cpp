...
namespace pe32 {
	...
	bool PEFile::open(const string& fileName) {
		...
		if (handle.get() == INVALID_HANDLE_VALUE) {
			throw Exception("PEFile::open", "Invalid handle value");
		}
		_fileName = fileName;
		...
		if (!ReadFile(handle.get(), _mem.data(), _fileSize, &dwNumberOfBytesRead, nullptr)) {
			throw Exception("PEFile::open", "Cannot read file data");
		}
		setPEHeaders();
		if (!isValidHeader()) {
			throw Exception("PEFile::open", "Invalid PE32 header");
		}
		return true;
	}
	...
	bool PEFile::save(const string& fileName) {
		...
		if (handle.get() == INVALID_HANDLE_VALUE) {
			throw Exception("PEFile::save", "Invalid handle value");
		}
		_fileName = fileName;
		DWORD dwNumberOfBytesWritten;
		if (!WriteFile(handle.get(), _mem.data(), _fileSize, &dwNumberOfBytesWritten, nullptr)) {
			throw Exception("PEFile::save", "Cannot write file data");
		}
		return true;
	}
